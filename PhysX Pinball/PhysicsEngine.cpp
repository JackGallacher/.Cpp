...
namespace PhysicsEngine
{
	...
	void Actor::Material(PxMaterial* new_material, PxU32 shape_index)
	{
		...
		for (PxU32 i = 0; i < shape_list.size(); i++)
		{
			...
			shape_list[i]->setMaterials(materials.data(), materials.size());
		}
	}
	...
	{
		std::vector<PxShape*> shapes(((PxRigidActor*)actor)->getNbShapes());
		((PxRigidActor*)actor)->getShapes((PxShape**)&shapes.front(),shapes.size());
		if (index == -1)
			return shapes;
	...
	void Scene::SelectNextActor()
	{
		...
		if (actors.size() && (px_scene->getActors(PxActorTypeSelectionFlag::eRIGID_DYNAMIC, (PxActor**)&actors.front(), actors.size())))
		{
			if (selected_actor)
			{
